...
while(decision!= 1){
  ...
  printf("Type 3 to watch AI vs AI \n ");
...
if(AI == 0 && computer == WHITE)
curr_position = AI_current(computer, game.board);
...
curr_position=Get_Current_Position(game.board);
  ...
  {
	if(AI == 0)
	destination = AI_current(computer,game.board);
	else
...
/*else if(AI == 1){ //with ai
	...
	if(computer == WHITE){
		...
		ai();
		printboard(game);
		log_move(file,curr_position,destination);
	}
...
{
curr_position=Get_Current_Position(game.board);
  if(curr_position->piece==NULL)
    printf("Invalid selection!. Please select a square with a piece.\n");

  else if(curr_position->piece!=NULL && curr_position->piece->color==WHITE)
  {
    destination=Get_New_Position(game.board);
    w_previous_current=curr_position;
    w_previous_destination=destination;
    ...
    if(curr_position->piece->piece_type==1 || curr_position->piece->piece_type==2)
        valid_move=Pmove(curr_position,destination,game.board,b_previous_current,b_previous_destination);

    else if(curr_position->piece->piece_type==3 || curr_position->piece->piece_type==4)
        valid_move=Rmove(curr_position,destination,game);

    else if(curr_position->piece->piece_type==5 || curr_position->piece->piece_type==6)
        valid_move=Hmove(curr_position,destination);

    else if(curr_position->piece->piece_type==7 || curr_position->piece->piece_type==8)
        valid_move=Bmove(curr_position,destination,game);

    else if(curr_position->piece->piece_type==9 || curr_position->piece->piece_type==10)
        valid_move=Qmove(curr_position,destination,game);

    else if(curr_position->piece->piece_type==11 || curr_position->piece->piece_type==12)
        valid_move=Kmove(curr_position,destination,game.board);
  }
  else
    printf("Invalid piece selection! Please select the correct color piece\n");
